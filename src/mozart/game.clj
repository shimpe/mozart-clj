...
(defn notes-on [sink notes vel]
  (doseq [n notes]
    (midi-note-on sink (pitch/note n) vel)))

(defn notes-off [sink notes]
  (doseq [n notes]
    (midi-note-off sink (pitch/note n))))

(defn notes-on-off [sink notes vel dur]
;;  (println notes)
  (notes-on sink notes vel)
  (at (+ (now) (* ALLOW-GAP dur))
    #(notes-off sink notes) at-pool))
